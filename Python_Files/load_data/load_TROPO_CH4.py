...
SOURCE = conn['data_source_ch4']    # Where to find the data
...
#            ../../SQL_Scripts/make_TROPOMI_tables/make_CH4_tables.sql " % 
...
cursor.execute('SELECT * FROM tropomi_CH4_files_loaded;')
...
for file in reversed(sorted(os.listdir(SOURCE))):
    ...
    if file.endswith('.zip') and file_num > START_NUM and file_num <= END_NUM: 
        ...
        if (file not in files_loaded):
            ...
            ch4_bias_corrected = np.array( nc_file.groups['PRODUCT']['methane_mixing_ratio_bias_corrected'][0,:,:]) #
            ch4_before_bias = np.array( nc_file.groups['PRODUCT']['methane_mixing_ratio'][0,:,:]) #(time,scanline,ground_pixel)
            ch4_precision = np.array( nc_file.groups['PRODUCT']['methane_mixing_ratio_precision'][0,:,:])  
            ...
            ch4 = ch4_bias_corrected[index[:,0], index[:,1]]
            ch4_raw = ch4_before_bias[index[:,0], index[:,1]]
            ch4_errs = ch4_precision[index[:,0], index[:,1]]
            ...
            for i in range(num_rows):
                ...
                args.append((date, float(ch4[i]), float(ch4_errs[i]), 
                             float(ch4_raw[i]), float(quality_flag[i]), 
            ...
            dbapi.extras.execute_values(cursor, "INSERT INTO tropomi_CH4 \
                (time, ch4, ch4_err, ch4_raw, quality_flag, \
                sza, vza, surf_p, x_prior, ak, center_pt, mbr) VALUES %s", 
                args)
