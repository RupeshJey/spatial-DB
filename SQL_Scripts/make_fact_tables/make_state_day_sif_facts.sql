-- Rupesh Jeyaram 
-- Created July 18th, 2019

-- DROP TABLE statements

DROP TABLE IF EXISTS state_day_sif_facts;

-- CREATE TABLE statements

-- state_day_sif_facts holds state-wise daily averages of SIF
-- Each record is a "fact", an aggregate stored measurement

CREATE TABLE state_day_sif_facts (

    -- The state that this fact is associated with 
    shape_id    INTEGER         NOT NULL,

    -- The day that this fact is associated with
    day         DATE            NOT NULL, 

    -- The average SIF value for this state-day combination
    -- This value *can* be NULL, when there are no measurements for 
    -- a state-day combo
    sif_avg     NUMERIC(5, 3),

    -- A fact is uniquely identified by a state-day combination
    -- There should not be any duplicates of these values
    PRIMARY KEY (shape_id, day)
);

-- CREATE INDEX statements 

-- Index on the date of fact
CREATE INDEX state_day_sif_facts_day_idx
  ON state_day_sif_facts(day);

-- POPULATE TABLE statements

-- Extract just the states
WITH states AS (
    SELECT * FROM shapes WHERE type = 'State'
),
-- Extract the day range we want (min to max)
day_range AS (
    SELECT date_trunc('day', dd):: date AS day
    FROM generate_series ( (SELECT MIN(time) FROM tropomi_SIF) 
                            , (SELECT MAX(time) FROM tropomi_SIF)
                            , '1 day'::interval) dd
)
-- Direct output into facts table
INSERT INTO state_day_sif_facts
-- Select out shape_id and day (to match the facts table)
SELECT shape_id, day, 
-- Obtain the average sif value for this 
(
    SELECT AVG(sif) 
    FROM tropomi_sif
    WHERE ST_X(center_pt) > -130 AND                       -- Exclude Alaska
          shape && center_pt AND                           -- Match state shape
          ST_CONTAINS(shape, center_pt) AND                -- Match shape  
          time BETWEEN day AND (day + INTERVAL '1 day')    -- Match day
) AS avg_sif
-- Cross states and days, so we have each possible combination (allows nulls)
FROM states CROSS JOIN day_range;

